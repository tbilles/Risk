...
public class ServerCommandVisitor implements CommandVisitor {
    ...
    public ServerCommandVisitor(CommandSender cmdSender, GameView gameView,
            GameController gameCtrl, ClientHandler clientHandler) {
    ...
    public void visit(HelloCmd cmd) {
        ...
        Player newPlayer = new Player(cmd.getName(), colors.get(gameView
                .getPlayers().size()));
    ...
    private void startGame() {
        ...
        while (!countries.isEmpty()) {
            ...
            Logger.logdebug("Country name: " + c.getName() + " to "
                    + c.getOwner().getName());
            c.setTroops(1);
    ...
    private void initNextRound() {
        ...
        cmdSender.sendCmd(
                new NextRoundCmd(gameView.getPlayers(), gameView
                        .getRoundPhases()), null);
        gotoNextPlayer();
    }
    ...
    public void visit(PlaceReinforcementCmd cmd) {
        ...
        Logger.logdebug("Got PlaceReinforcement command to "
                + country.getName() + " (" + cmd.getTroops() + ")");
        if (clientHandler.getPlayer() != gameView.getCurrentPlayer()) {
            Logger.logwarn(clientHandler.getPlayer().getName()
                    + " is trying to place reinforcement in someone else's turn "
                    + gameView.getCurrentPlayer().getName());
            sendError(ErrorCmd.ILLEGAL_ARGUMENT);
            return;
        }

        if (cmd.getTroops() < 0
                || cmd.getTroops() > gameView.getAvailableReinforcement()) {
            Logger.logwarn(clientHandler.getPlayer().getName()
                    + " is trying to place " + cmd.getTroops()
                    + " reinforcement, available is "
                    + gameView.getAvailableReinforcement());
            sendError(ErrorCmd.ILLEGAL_ARGUMENT);
            return;
        }

        if (gameView.getCountry(cmd.getCountry().getName()).getOwner() != clientHandler
                .getPlayer()) {
            Logger.logwarn(clientHandler.getPlayer().getName()
                    + " is trying to place reinforcement to "
                    + country.getName() + " (" + country.getOwner().getName()
                    + ")");
            sendError(ErrorCmd.ILLEGAL_ARGUMENT);
            return;
        }

        gameCtrl.setAvailableReinforcement(gameView.getAvailableReinforcement()
                - cmd.getTroops());
        gameCtrl.addTroopsToCountry(country, cmd.getTroops());
        cmdSender.sendCmd(new PlaceReinforcementCmd(country, cmd.getTroops(),
                gameView.getCurrentPlayer()), null);
    ...
    public void visit(RegroupCmd cmd) {
        ...
        Logger.logdebug("Got regroup command " + from.getName() + " -> "
                + to.getName() + " : " + cmd.getTroops());
        if (from.getOwner() != to.getOwner()
                || from.getOwner() != clientHandler.getPlayer()) {
            Logger.logerror("Invalid regroup command, owners do not match");
            sendError(ErrorCmd.ILLEGAL_ARGUMENT);
            return;
        }
        if (!((gameView.getRoundPhase() == RoundPhase.ATTACK && gameView
                .getNextRoundPhase() == RoundPhase.REGROUP) || gameView
                .getRoundPhase() == RoundPhase.REGROUP)) {
    ...
    public void visit(AttackStartCmd cmd) {
        ...
        Logger.logdebug("Got AttackStart command " + from.getName() + " -> "
                + to.getName());

        if (from.getOwner() != clientHandler.getPlayer()
                || to.getOwner() == clientHandler.getPlayer()) {
        ...
        if (gameView.getRoundPhase() != RoundPhase.ATTACK) {
            Logger.logerror("Attack start in wrong phase! ("
                    + gameView.getRoundPhase() + ")");
            sendError(ErrorCmd.INVALID_PHASE);
            return;
        }
    ...
    public void visit(AttackSetADiceCmd cmd) {
        ...
        if (!(dice >= 1 && dice <= 3 && dice < from.getTroops())) {
            Logger.logdebug("Too many (" + dice + ") adice! (troops: "
                    + from.getTroops() + ")");
            sendError(ErrorCmd.ILLEGAL_ARGUMENT);
            return;
        }
    ...
    public void visit(AttackSetDDiceCmd cmd) {
        ...
        if (!(dice >= 1 && dice <= 2 && dice <= to.getTroops())) {
            Logger.logdebug("Too many (" + dice + ") ddice! (troops: "
                    + to.getTroops() + ")");
            sendError(ErrorCmd.ILLEGAL_ARGUMENT);
            return;
        }
